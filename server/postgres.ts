...
export async function runPostgresQuery(sql: string, connection: DBConnection) {
  ...
  try {
    ...
    return { columns, rows, rowCount: rows.length, executionTime: Date.now() - start }
  } finally {
    await client?.end()
  }
}
...
export async function getPostgresColumns(connection: DBConnection, tableName: string) {
  let client
  try {
    client = await getClient(connection)
    const result = await client.query(
