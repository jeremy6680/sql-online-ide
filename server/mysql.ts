...
function extractLastResult(rows: unknown, fields: unknown): { lastRows: unknown; lastFields: FieldPacket[] | undefined } {
  ...
  const isMulti = Array.isArray(rows) && rows.length > 0 &&
    (Array.isArray(rows[0]) || (typeof rows[0] === 'object' && rows[0] !== null && 'fieldCount' in (rows[0] as object) && !('constructor' in (rows[0] as object && (rows[0] as Record<string, unknown>)))))

  if (isMulti) {
    const allRows = rows as unknown[]
    const allFields = (Array.isArray(fields) ? fields : []) as (FieldPacket[] | undefined)[]
  ...
  return { lastRows: rows, lastFields: Array.isArray(fields) && fields.length > 0 && (fields[0] as FieldPacket)?.name !== undefined ? fields as FieldPacket[] : undefined }
}
  ...
  try {
    ...
    if (lastFields && Array.isArray(lastFields) && lastFields.length > 0) {
      ...
      const data = Array.isArray(lastRows)
        ? (lastRows as Record<string, unknown>[]).map(r => columns.map(c => r[c]))
        : []
      return { columns, rows: data, rowCount: data.length, executionTime: Date.now() - start }
    } else {
      const result = (Array.isArray(lastRows) ? lastRows[lastRows.length - 1] : lastRows) as { affectedRows?: number }
      return { columns: ['affected_rows'], rows: [[result?.affectedRows ?? 0]], rowCount: 1, executionTime: Date.now() - start }
    }
  } finally {
    await conn?.end()
  }
...
export async function getMySQLTables(connection: DBConnection) {
  ...
  try {
    ...
    return (rows as unknown as { TABLE_NAME: string; TABLE_TYPE: string }[]).map(r => ({
      name: r.TABLE_NAME,
...
export async function getMySQLColumns(connection: DBConnection, tableName: string) {
  let conn
  try {
    conn = await getConnection(connection)
    const [rows] = await conn.execute(
      ...
      [tableName]
    ) as [{ COLUMN_NAME: string; DATA_TYPE: string; IS_NULLABLE: string; COLUMN_KEY: string }[][], unknown]
    return (rows as unknown as { COLUMN_NAME: string; DATA_TYPE: string; IS_NULLABLE: string; COLUMN_KEY: string }[]).map(r => ({
      name: r.COLUMN_NAME,
      type: r.DATA_TYPE,
