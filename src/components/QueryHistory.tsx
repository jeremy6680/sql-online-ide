  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--ide-border)]">
        <span className="text-xs text-[var(--ide-text-2)]">{t('history.entries', { count: history.length })}</span>
        <button
          onClick={onClear}
          className="flex items-center gap-1 text-xs text-red-400 hover:text-red-300"
        >
          <Trash2 size={11} /> {t('history.clear')}
        </button>
      </div>
        ...
        {history.map((entry) => (
          ...
          >
            ...
            {entry.rowCount !== undefined && (
              <div className="text-xs text-[var(--ide-text-4)] mt-0.5">{t('history.rows', { count: entry.rowCount })}</div>
            )}
          </div>
        ))}
      </div>
