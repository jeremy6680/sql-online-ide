...
export async function runSQLiteQuery(sql: string): Promise<QueryResult> {
  ...
  try {
    ...
    const statements = sql.split(';').map(s => s.trim()).filter(s => {
      ...
      if (upper.includes('ALTER TABLE') && upper.includes('ADD FOREIGN KEY')) return false
      if (upper.includes('ALTER TABLE') && upper.includes('ADD CONSTRAINT') && upper.includes('FOREIGN KEY')) return false
      return true
    })
    let lastResult: QueryResult = { columns: [], rows: [], rowCount: 0, executionTime: 0 }
