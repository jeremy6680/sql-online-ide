...
- [ ] **Named query tabs / multi-buffer** — allow multiple open queries, switchable via tabs. Essential for comparing queries or working on several at once.
- [ ] **Import CSV/JSON/Parquet into DuckDB** — DuckDB can query these file formats natively. A dedicated import flow (drag & drop → auto-`COPY FROM`) would be a flagship DuckDB feature.
...
- [ ] **Execution plan viewer** — for SQLite (`EXPLAIN QUERY PLAN`) and DuckDB (`EXPLAIN ANALYZE`), parse and display the output in a readable tree or diagram.
- [ ] **Query formatting (Prettier SQL)** — a "Format" button that reformats the SQL in the editor using a SQL formatter library.
- [ ] **Share query via URL** — encode the current SQL + engine in the URL hash so queries can be shared as links.
